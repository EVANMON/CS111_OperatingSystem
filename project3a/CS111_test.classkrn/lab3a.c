...
void inode_summary() {
    ...
    for (j=0; j<(int) inode_num; ++j) {
        ...
        pread(fd, &inode, sizeof(struct ext2_inode), 5*1024+j*sizeof(struct ext2_inode));
    	if (inode.i_mode != 0x0 && inode.i_links_count != 0x0) {
    	...
    	fprintf(stdout, "INODE,%u,%c,%o,%d,%u,%u,%s,%s,%s,%u,%u", j+1, \
